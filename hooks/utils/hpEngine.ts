 ...
 *  - HP is driven by task HP costs, not a fixed daily goal
 *  - Formula: HP = (sum of completed task hpCosts / sum of all task hpCosts) × 100
 *  - If no tasks exist, HP defaults to 100 (nothing to do = Ember is happy)
...
import { EmberState } from "@/types";
import { BONFIRE_HP_THRESHOLD } from "@/constants/EmberStates";

/**
 * Calculates Ember's HP based on completed vs total task HP costs.
 *
 * Formula: HP = (completedHP / totalHP) × 100
 *   - completedHP: sum of hpCost from all completed tasks
 *   - totalHP: sum of hpCost from all tasks (completed + incomplete)
 *   - No tasks → 100 HP (Ember starts happy)
 *   - All tasks completed → 100 HP (Bonfire territory)
 *   - HP decreases as new tasks are added, increases as tasks are completed
 */
export function calculateHP(completedHP: number, totalHP: number): number {
  // No tasks on the board — Ember is content
  if (totalHP <= 0) {
    return 100;
  }
  return Math.min((completedHP / totalHP) * 100, 100);
}
